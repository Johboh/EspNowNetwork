...
/**
 * copied from https://github.com/Locoduino/RingBuffer and the arduino.h dependancy has been removed
 * along with the ISR-specific methods that used it. 
 */
...
template <typename ET, size_t S, typename IT, typename BT>
void RingBuf<ET, S, IT, BT>::incReadIndex() {
  mReadIndex++;
  if (mReadIndex == S)
    mReadIndex = 0;
}
